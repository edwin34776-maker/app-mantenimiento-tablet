    ...
    if not df.empty:
        ...
        with col_torta:
            if "Estado" in df.columns:
                ...
                total_est = estados_df["Cantidad"].sum()
                pct_pend = estados_df[estados_df["Estado"]=="Pendiente"]["Cantidad"].values[0] / total_est * 100 if total_est else 0
                pct_ejec = estados_df[estados_df["Estado"]=="Ejecutado"]["Cantidad"].values[0] / total_est * 100 if total_est else 0
                pct_verif = estados_df[estados_df["Estado"]=="Verificado"]["Cantidad"].values[0] / total_est * 100 if total_est else 0
                st.markdown(f"""
                <div style="text-align: center;">
                    <svg width="180" height="180" viewBox="0 0 180 180" style="margin: 0 auto;">
                        <circle cx="90" cy="90" r="70" fill="#F59E0B" stroke="white" stroke-width="2"/>
                        <circle cx="90" cy="90" r="70" fill="transparent" stroke="#22C55E" stroke-width="70" 
                            stroke-dasharray="{pct_ejec/100*440:.1f} 440" transform="rotate(-90 90 90)"/>
                        <circle cx="90" cy="90" r="70" fill="transparent" stroke="#3B82F6" stroke-width="70" 
                            stroke-dasharray="{pct_verif/100*440:.1f} 440" 
                            stroke-dashoffset="{-pct_ejec/100*440:.1f}" transform="rotate(-90 90 90)"/>
                        <circle cx="90" cy="90" r="45" fill="white"/>
                        <text x="90" y="82" text-anchor="middle" font-size="11" fill="#64748B" font-weight="600">Total</text>
                        <text x="90" y="102" text-anchor="middle" font-size="22" fill="#0F172A" font-weight="900">{total_est}</text>
                    </svg>
                    <div style="display: flex; justify-content: center; gap: 12px; margin-top: 8px;">
                        <div style="display: flex; align-items: center; gap: 4px;"><div style="width: 10px; height: 10px; border-radius: 2px; background: #F59E0B;"></div><span style="font-size: 11px; color: #475569;">Pendiente <strong>{estados_df[estados_df['Estado']=='Pendiente']['Cantidad'].values[0]}</strong></span></div>
                        <div style="display: flex; align-items: center; gap: 4px;"><div style="width: 10px; height: 10px; border-radius: 2px; background: #22C55E;"></div><span style="font-size: 11px; color: #475569;">Ejecutado <strong>{estados_df[estados_df['Estado']=='Ejecutado']['Cantidad'].values[0]}</strong></span></div>
                        <div style="display: flex; align-items: center; gap: 4px;"><div style="width: 10px; height: 10px; border-radius: 2px; background: #3B82F6;"></div><span style="font-size: 11px; color: #475569;">Verificado <strong>{estados_df[estados_df['Estado']=='Verificado']['Cantidad'].values[0]}</strong></span></div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with col_barras:
            if "Especialidad" in df.columns and "Estado" in df.columns:
                ...
                pivot_df = stack_df.pivot(index="Especialidad", columns="Estado", values="Cantidad").fillna(0)
                for est in ["Pendiente", "Ejecutado", "Verificado"]:
            ...
            if "Estado" in df.columns:
                ...
                for est in ["Pendiente", "Ejecutado", "Verificado"]:
                    if est not in estados_df["Estado"].values:
                        estados_df = pd.concat([estados_df, pd.DataFrame({"Estado": [est], "Cantidad": [0]})], ignore_index=True)
                total_est = estados_df["Cantidad"].sum()
                c_pend = estados_df[estados_df["Estado"]=="Pendiente"]["Cantidad"].values[0] if "Pendiente" in estados_df["Estado"].values else 0
                c_ejec = estados_df[estados_df["Estado"]=="Ejecutado"]["Cantidad"].values[0] if "Ejecutado" in estados_df["Estado"].values else 0
                c_verif = estados_df[estados_df["Estado"]=="Verificado"]["Cantidad"].values[0] if "Verificado" in estados_df["Estado"].values else 0
                p_pend = c_pend / total_est * 100 if total_est else 0
                p_ejec = c_ejec / total_est * 100 if total_est else 0
                p_verif = c_verif / total_est * 100 if total_est else 0
                st.markdown(f"""
                <div style="text-align: center;">
                    <svg width="180" height="180" viewBox="0 0 180 180" style="margin: 0 auto;">
                        <circle cx="90" cy="90" r="70" fill="#F59E0B" stroke="white" stroke-width="2"/>
                        <circle cx="90" cy="90" r="70" fill="transparent" stroke="#22C55E" stroke-width="70" 
                            stroke-dasharray="{p_ejec/100*440:.1f} 440" transform="rotate(-90 90 90)"/>
                        <circle cx="90" cy="90" r="70" fill="transparent" stroke="#3B82F6" stroke-width="70" 
                            stroke-dasharray="{p_verif/100*440:.1f} 440" 
                            stroke-dashoffset="{-p_ejec/100*440:.1f}" transform="rotate(-90 90 90)"/>
                        <circle cx="90" cy="90" r="45" fill="white"/>
                        <text x="90" y="82" text-anchor="middle" font-size="11" fill="#64748B" font-weight="600">Total</text>
                        <text x="90" y="102" text-anchor="middle" font-size="22" fill="#0F172A" font-weight="900">{total_est}</text>
                    </svg>
                    <div style="display: flex; justify-content: center; gap: 12px; margin-top: 8px;">
                        <div style="display: flex; align-items: center; gap: 4px;"><div style="width: 10px; height: 10px; border-radius: 2px; background: #F59E0B;"></div><span style="font-size: 11px; color: #475569;">Pendiente <strong>{c_pend}</strong></span></div>
                        <div style="display: flex; align-items: center; gap: 4px;"><div style="width: 10px; height: 10px; border-radius: 2px; background: #22C55E;"></div><span style="font-size: 11px; color: #475569;">Ejecutado <strong>{c_ejec}</strong></span></div>
                        <div style="display: flex; align-items: center; gap: 4px;"><div style="width: 10px; height: 10px; border-radius: 2px; background: #3B82F6;"></div><span style="font-size: 11px; color: #475569;">Verificado <strong>{c_verif}</strong></span></div>
                    </div>
                </div>
                """, unsafe_allow_html=True)

        with col_barras:
            # Gráfica de barras apiladas por especialidad y estado
            if "Especialidad" in df.columns and "Estado" in df.columns:
                stack_df = df.groupby(["Especialidad", "Estado"]).size().reset_index(name="Cantidad")
                stack_df = stack_df[stack_df["Especialidad"].isin(["ELE", "MEC"])]
                pivot_df = stack_df.pivot(index="Especialidad", columns="Estado", values="Cantidad").fillna(0)
