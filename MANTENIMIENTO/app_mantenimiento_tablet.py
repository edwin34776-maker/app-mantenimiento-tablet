        ...
        with col_torta:
            if "Estado" in df.columns:
                ...
                pie = alt.Chart(estados_df).mark_arc(innerRadius=45, outerRadius=80).encode(
                    ...
                    color=alt.Color("Estado:N", scale=color_scale, legend=alt.Legend(orient="bottom", direction="horizontal")),
    ...
    with col_der:
        ...
        if not df_asig.empty and "Ubicacion" in df_asig.columns:
            ...
            if equipos_tec:
                ...
                bar_eq = alt.Chart(df_chart).mark_bar(cornerRadiusEnd=4).encode(
                    x=alt.X("sum(Actividades):Q", title="Actividades"),
                    y=alt.Y("Equipo:N", title=None, sort="-x"),
                    color=alt.Color("Técnico:N", legend=alt.Legend(orient="bottom", direction="horizontal", title=None)),
                    tooltip=["Equipo", "Técnico", "Actividades"]
                ).properties(height=alt.Step(28))
                st.altair_chart(bar_eq.configure_view(strokeWidth=0), use_container_width=True)
            else:
